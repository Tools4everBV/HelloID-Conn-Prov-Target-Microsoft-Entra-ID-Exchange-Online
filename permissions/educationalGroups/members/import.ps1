...
try {
    ...
    $educationalGroupMembersRequests = @(foreach ($group in $educationalGroups) {
        @{
            Method = 'GET'
            Uri    = "/groups/$($group.id)/members/microsoft.graph.user?`$select=id&`$top=999"
        }
    })
    $educationalGroupMembers = Invoke-MSEntraBatchRequest -Requests $educationalGroupMembersRequests -Headers $headers
    ...
    for ($i = 0; $i -lt $educationalGroups.Count; $i++) {
        ...
        $accountReferences = ($educationalGroupMembers[$i] | Where-Object { $_.id -ne $actionContext.Configuration.ownerGuid }).id
