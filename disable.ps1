...
try {
    ...
    $selectPropertiesToGetUser =  ($actionContext.Data | Select-Object * -ExcludeProperty ExchangeOnline).PSObject.Properties.Name -join ','
    $splatGetEntraUser = @{
        Uri     = "https://graph.microsoft.com/v1.0/users/$($outputContext.References.Account)?`$select=$selectPropertiesToGetUser"
        Method  = 'GET'
